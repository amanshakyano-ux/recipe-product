...
const createdBySection = document.getElementById("createdBySection");
...
const loadRecipeDetails = async () => {
  try {
    const response = await axios.get(`/api/recipes/get/${recipeId}`, {
      headers: {
        Authorization: token,
      },
    });
      ...
      <div class="recipe-card">
        ...
        ${recipe.user ? `
          <div class="creator-info-inline">
            <p><b>Created By:</b> <span class="creator-name-inline" onclick="viewUserProfile(${recipe.user.id})">
              ${recipe.user.name}
            </span></p>
          </div>
        ` : ''}
    ...
    createdBySection.innerHTML = '';

    showReviews(recipe.reviews);
  } catch (err) {
    console.log(err.response?.data || err.message);

    recipeDetails.innerHTML = "<h3>Failed to load recipe</h3>";
  }
};
...
const showReviews = (reviews) => {
  ...
  if (!reviews || reviews.length === 0) {
    reviewsContainer.innerHTML = "<h3>No reviews yet</h3>";
  ...
  try {
    if (user.role === "admin") {
      const response = await axios.delete(
        `/api/admin/deleteReview/${reviewId}`,
        {
          headers: {
            Authorization: token,
          },
